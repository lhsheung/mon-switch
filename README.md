...
- **開關一按即生效**：設定視窗按「套用」時即會寫入或移除登錄檔，不需要重新啟動程式，也不需要登出登入。
- **啟動時雙向同步**：程式啟動時會比對登錄檔與設定值。若設定為開啟但登錄檔沒有（或被防毒清除），會自動重寫；若登錄檔指向的是**另一個位置的舊副本**（程式被搬過資料夾），亦會重寫成目前路徑。
- **設定視窗顯示實際狀態**，不是只顯示設定值：已啟用／未啟用／指向另一個位置／已被 Windows 停用，並附「開啟設定檔資料夾」捷徑方便核對。
- 寫入失敗（極少見，例如政策鎖定登錄檔）會回退設定值並用當前語言提示使用者。

### Windows 另有一個獨立的「停用」開關

只看 `Run` 鍵會得出錯誤結論。Windows 在

```
HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\StartupApproved\Run
```

為每一個啟動項另存一個 12 位元組的二進位值，第一個位元組代表是否允許執行：`0x02`（舊版為 `0x06`）表示允許，`0x03` 表示停用。**工作管理員 → 啟動**以及**設定 → 應用 → 啟動**改的就是這個值。

當它被設為停用時，`Run` 鍵仍然完整存在，但 Windows 在登入時**直接跳過該項目**。表現出來就是「明明設定了開機自啟，卻從來沒有啟動」。因此：

- 設定視窗把這個狀態單獨顯示為「已啟用，但已被 Windows 的『啟動應用程式』停用」，而不是誤報為「已啟用」；
- 使用者在設定視窗明確開啟自啟時（按「套用」），程式會一併移除該停用標記，令開關真正生效；
- 但程式**不會**在每次啟動時擅自清除它 —— 那會覆蓋使用者在工作管理員裡刻意作出的選擇。
...
9. **開機自啟使用登錄檔 `Run` 鍵。** 部份企業政策或第三方「啟動管理」工具可能會把它刪除，程式下次啟動時會偵測到不一致並嘗試還原。另外，Windows 在 `StartupApproved` 之下還有一個獨立的停用標記（見 [第 5 節](#5-開機自動啟動方案)）；設定視窗會把它顯示出來，按「套用」即會清除。
...
| 7 | 開機自啟可開關，重啟後生效，自啟時靜默到托盤 | 在設定視窗勾選後按「套用」，**不需重啟程式**，`HKCU\...\Run` 應立即出現 `mon-switch` 值且帶 `--silent`，下方狀態應轉為「已啟用」；登出再登入，托盤圖示應出現且無任何視窗彈出 | 寫入與狀態已驗證；登出登入那一步待人工 |
