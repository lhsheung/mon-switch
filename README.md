...
兩者都提供：安裝精靈、開始功能表捷徑（含一個「Uninstall mon-switch」）、在「設定 → 應用」中的卸載項，以及完成頁「立即啟動 mon-switch」選項（預設勾選）。
...
已實測（per-user 套件，全程無 UAC 提示）：靜默安裝 → 4 個檔案就位、開始功能表捷徑建立、登錄標記寫入、「設定 → 應用」出現卸載項；靜默卸載 → 檔案、捷徑、登錄標記全部清除。
